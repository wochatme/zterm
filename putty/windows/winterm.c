...
HWND PTerm_CreateWindow(HWND hWndParent)
{
    ...
    exwinmode = 0;
    winmode = WS_VSCROLL | WS_CHILD | WS_VISIBLE;
    wgs.term_hwnd = CreateWindowExW(
        exwinmode, terminal_window_class_w(), uappname,
        winmode, CW_USEDEFAULT, CW_USEDEFAULT,
        guess_width, guess_height, hWndParent, NULL, hinst, NULL);
    ...
    {
        term->blink_cur = true;
        term->curstype = 2; /* 0=block 1=underline 2=vertical */
        //term->curstype = 0; /* 0=block 1=underline 2=vertical */
...
BOOL PTerm_before_loop(MSG* msg)
{
    ...
    if (toplevel_callback_pending() || bRet) {
        /*
...
{
    unsigned int wlen = 0;
    wchar_t* screen_buf = ZT_copyScreen(term, clips_system, lenof(clips_system));

    if (screen_buf && caller_buff)
    {
        wlen = (unsigned int)wcslen(screen_buf);
        if (wlen <= max_length)
        {
...
int PTerm_do_settings(HWND hWndParent)
{
#if 10
    int resize_action;
...
#if 0
int WINAPI xxxWinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
    MSG msg;
    HRESULT hr;
    int guess_width, guess_height;
