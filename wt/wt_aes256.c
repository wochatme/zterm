...
static void wt_AES256_init(AES256_ctx* ctx, const unsigned char* key32) 
{
...
/*
 * Please check the idea behind the below two functions in 
...
void wt_AES256_encrypt32Bytes(unsigned char* key32, unsigned char* iv16, unsigned char* cipher32, const unsigned char* plain32)
{
    ...
    for (i = 0; i < 16; i++) tmpbuf[i] = iv16[i] ^ plain32[i];
    AES_encrypt(ctx->rk, 14, cipher32, tmpbuf);

    for (i = 0; i < 16; i++) tmpbuf[i] = cipher32[i] ^ plain32[i + 16];
    AES_encrypt(ctx->rk, 14, cipher32 + 16, tmpbuf);
...
void wt_AES256_decrypt32Bytes(unsigned char* key32, unsigned char* iv16,  const unsigned char* cipher32, unsigned char* plain32)
{
    ...
    AES_decrypt(ctx->rk, 14, tmpbuf, cipher32);
    for (i = 0; i < 16; i++) plain32[i] = tmpbuf[i] ^ iv16[i];

    AES_decrypt(ctx->rk, 14, tmpbuf, cipher32 + 16);
    for (i = 0; i < 16; i++) plain32[i+16] = tmpbuf[i] ^ cipher32[i];
}
